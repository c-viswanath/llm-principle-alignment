...
# Model Name (Change to phi3:14b, qwen2.5:32b, etc.)
MODEL_NAME = "phi3:14b"
...
# ================= PRINCIPLES DICTIONARY (CRITICAL) =================
# The script will use these texts when it sees F1, F2, etc. in 'principle_id'
prompts_dict = {
...
def get_messages(mode, q, r1, r2, principle, level="standard"):
    # 1. Data Block (Handle missing question)
...
def run_pipeline():
    ...
    for i in tqdm(range(len(df))):
        ...
        # --- CRITICAL FIX: PRINCIPLE MAPPING LOGIC ---
        # 1. Try to find the ID (e.g., F1) in principle_id or prompt_key
        p_id = str(row.get("principle_id", row.get("prompt_key", ""))).strip().upper()
        
        # 2. Look up the ID in the dictionary
        if p_id in prompts_dict:
            principle = prompts_dict[p_id]
        else:
            # 3. If ID not found, try to use the description column directly
